...
Start the Java service with workers disabled until the final import is verified.
"""
import argparse
import json
import os
...
def prepare(output):
    source = {**dotenv_values(ROOT / "backend/.env"), **dotenv_values(ROOT / "backend/.env.docker")}
    source = {key: value for key, value in source.items() if value is not None}
    secret = source.get("JWT_SECRET_KEY", "")
    if len(secret.encode()) < 32:
        raise RuntimeError("Legacy JWT key is weak; explicitly choose a compatible key policy before deployment")
    ...
    business = {**postgres, "JWT_SECRET_KEY": secret, "INTERNAL_SERVICE_KEY": internal,
        ...
        "UPLOAD_DIR": "/data/uploads", "WORKERS_ENABLED": "false", "APP_ENV": "development"}
    ...
    agent.update(INTERNAL_SERVICE_KEY=internal, BUSINESS_URL="http://backend:9000", DATA_DIR="/app/data",
        CHROMA_DIR="/app/data/chroma", APP_ENV="development", ACCEPTANCE_BUDGET_FILE="/app/runtime/acceptance-budget.sqlite3")
    output = output.resolve()
    output.mkdir(parents=True, exist_ok=False)
    for name, values in (("postgres",postgres),("business",business),("agent",agent)):
    ...
    print(json.dumps({"private_config": str(output), "workers_enabled": False, "agent_has_business_credentials": False}))
