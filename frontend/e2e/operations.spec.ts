...
test('saved trip supports checks, expense ledger and accepted editor changes', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: '核验景点营业信息' }).click()
  ...
  await expect(page.getByText('editor · pending')).toBeVisible()
