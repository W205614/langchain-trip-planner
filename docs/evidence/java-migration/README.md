...
用户刷新后的现有访问日志确认主景区图片已返回 36,396 字节，子点位仍为占位图。按用户要求统一更新图片链路，没有按景点名称硬编码：实际 ID 原图候选重试；无图时仅接受同城、3 公里内、名称前缀严格匹配的唯一景区候选，内嵌“景区参考图（非该具体点位实拍）”标注。图片需真实解码通过，拒绝损坏文件和远程 SVG；短失败缓存、有限容量成功缓存、24 秒截止时间传播；前端更新图片缓存版本并保留完整画面及标注。新增 12 项离线回退测试和 1 项损坏 JPEG 用例，保留原 HTTP 升级/格式识别检查。日常 Agent/前端已重建；没有为图片追加真实高德请求，所有历史点位的真实图片覆盖率仍需用户刷新验证，不能声称每个点位都有实拍。
