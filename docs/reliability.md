...
真实模型评测默认关闭。独立评测进程设置 `LIVE_EVAL_ENABLED=true`、调用上限、美元预算、输入/输出单价，再以管理员 EVAL_TOKEN 运行 benchmark 的 `--live` 模式。预留费用按提示字节与最大输出保守估计，计数是进程级，重启会重置；供应商账单与价格仍需人工核对。不要在普通 CI 中放真实密钥。本轮未运行付费模型评测。
