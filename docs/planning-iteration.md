...
`PlanningConstraints` 保存在任务请求和最终 plan JSON 中，历史编辑从服务器原记录还原，不能用客户端传来的空约束覆盖。名称解析已升级：支持城市前缀、城市限定常用别名和唯一候选名称变体；多个分馆等同级歧义不静默选取，不以简单子串替代主体景点。详见 [显示修复](evidence/trip-display-fixes.md)。相互冲突的必去/不去请求返回 422。必去名称未查到或未选入时明确报告，不能虚构 POI。
...
每日总时间包括景点游览、景点间导航、90 分钟用餐预留和30分钟缓冲。它不包含已验证的餐厅/酒店路线或预约时段。公交接口未提供步行分段时，步行上限标为未核实。开放时间已回填高德查询参考，但出行日开放状态、预约、景区内部步行和父子景区关系仍没有完整事实覆盖，不能宣称整份行程已经可执行。
